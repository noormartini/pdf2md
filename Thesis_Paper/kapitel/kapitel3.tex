...
Ouyang et al.\ \autocite{omnidocbench2025} evaluated these tools across diverse
real-world document layouts and found no single system performs consistently well
...
specialised tools. Vision-capable language models changed this: they read a
...
example: Meuschke et al.\ \autocite{meuschke2023} confirmed that structured-output
tools of this kind outperformed all rule-based parsers on scientific documents.
...
Persistent gaps remain across all approaches: word-level representations miss
document-level semantic structure, and multi-page documents with mixed content
are still an open challenge \autocite{ding2026}.
...
\textbf{This work} & both   & Partial & Yes & \textbf{Yes} & Markdown \\
\bottomrule
\end{tabularx}
\end{table}

The \textit{both} label for this work reflects the image-path strategies: text-only pages go through PyMuPDF's text layer, while all other page types are rendered as PNG images and passed to the vision-language model.
A scanned PDF would follow the same image path and the VLM would read the rendered scan.
However, the page classifier relies on text-layer signals (character count, table detection, formula detection), which are absent in scanned documents.
The evaluation in this thesis covers native PDFs only, and the classifier thresholds were calibrated on native content.
