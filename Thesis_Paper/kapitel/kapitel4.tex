...
\begin{figure}[ht]
    \centering
    \input{tikz/eval_pipeline.tikz}
    \caption{Evaluation pipeline: corpus, strategies, ground truth, metrics, mean scores, and the paired permutation test}
...
The test corpus consists of bachelor thesis documents. This scope
...
primarily in English, with one document in German, reflecting the intended
usage context. Each selected document contains at least one structurally challenging
...
pages, 30 per document. All three belong to a single document category:
academic bachelor theses. This single-category design keeps the
comparison focused on the intended use case but limits the scope of the
results to this document type. Three documents is a small corpus. The
results support strategy comparisons within this set, but are not sufficient
for general claims about other document types, multilingual content, or
different academic formats. This limitation is discussed further in
Chapter~8.
...
skipping levels that are absent in both. List structure score compares the
number of list items, bulleted and numbered combined, between candidate
and reference. Paragraph
...
Score differences between strategies are tested for statistical significance
with a paired sign-flip permutation test. Every page is converted by all four
strategies, so per-page scores form matched pairs, and the test asks how often
a mean difference at least as large as the observed one would arise if each
page's difference were equally likely to carry either sign. The test is
two-sided, uses 100{,}000 resamples, and treats $p < 0.05$ as significant.
It makes no assumption about how the scores are distributed, which matters
for metrics where most pages tie and only a handful differ.
...
Qwen2.5-VL~7B, served through LM Studio on a rented cloud GPU rather than
local hardware. The model runs locally just as well, but a full 360-page
run on local hardware takes hours; the same run completes in minutes on
the rented GPU, which is why it was used here. The text strategy does not
use a model at all. It calls
PyMuPDF directly through the pymupdf4llm library, which converts the PDF
text layer to Markdown without invoking any LLM. The three vision-based
strategies therefore run under identical hardware and model conditions,
which keeps that part of the comparison fair. Text-only's processing time
instead reflects local disk and CPU speed, since it never reaches the GPU
at all. Each strategy's output is scored after the full postprocessing
pipeline described in Section~\ref{sec:postprocessing_pipeline}, so the
metrics in Chapter~6 measure what the deployed system actually returns to
a user, not an earlier, unpolished stage of it.
...
Results are reported as mean scores per document across all pages in that
document. Meuschke et al.\ \autocite{meuschke2023} used the same
document-level reporting and found that tools which rank well on one
document often rank poorly on another. Document-level means make this
