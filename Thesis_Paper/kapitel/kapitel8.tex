...
predicted. Three of the four hypotheses needed revision on this corpus. The reason, examined in
...
LLM call. The small fixed classification cost is negligible compared to the
model inference it avoids on most pages. On longer documents the gap
...
temperature setting. Section~\ref{sec:threats_to_validity} reports informal testing
with a larger model, which found that part of this gap is a fixable extraction bug
rather than a model limit.
The heading structure metric is consistently the weakest across all four
...
more than any other single change. Section~\ref{sec:practical_implications} identifies the table classifier as
conservative: all detected tables are routed to the VLM regardless of complexity.
Building and evaluating a table complexity check would show whether simple
row-column tables can stay in the text path without losing table structure score,
and how much that reduces token usage on table-heavy documents.
...
All VLM results in this experiment also depend on a single 7B model.
Section~\ref{sec:threats_to_validity} shows that switching to a larger model does
not close the whole gap to text-only: wide tables and source code still come out
wrong regardless of model size. A controlled comparison across model sizes, run
after the extraction bugs found in that test are fixed, would isolate how much of
the remaining gap is model capability rather than the pipeline around it.

The evaluation covers three bachelor theses, two in German and one in English. Testing on research papers,
