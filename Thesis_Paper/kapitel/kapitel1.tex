...
document collections, this is a serious bottleneck. In the RAG experiments of
Khan et al.\ \autocite{khan2024}, the extraction step, not the language model,
...
The choice matters for downstream tasks. Markdown preserves hierarchy through
headings, lists and tables, which means the document's structure is visible to
the embedding model and to any downstream reader. A plain paragraph dump does
not provide that.
...
Meuschke et al.'s finding that no single tool performs consistently well
across document types \autocite{meuschke2023} directly motivates the
adaptive approach: if the best tool for one
...
hold consistently across different thesis documents, or does
...
on bachelor thesis documents in German and English. The system
architecture is compatible with any LLM backend that supports a vision
Application Programming Interface (API). All experiments use LM Studio as
a self-hosted runtime rather than a third-party cloud AI service, which
keeps results reproducible regardless of which machine LM Studio runs on;
Section~4.4 describes the hardware setup used for the experiments.
