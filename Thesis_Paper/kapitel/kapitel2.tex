...
Figure~\ref{fig:extraction_example} shows two of these effects on a real
page from the corpus used in Chapter~4: the chapter heading arrives as two
plain text lines with nothing marking them as a heading, and words are cut
in half wherever the print layout hyphenated them at a line end.

\begin{figure}[ht]
\medskip\noindent\textbf{Raw sequential extraction (PyMuPDF)}
...
\medskip\noindent\textbf{The structure the reader perceives (ground-truth Markdown)}
\begin{lstlisting}[language={},nolol,identifierstyle=\color{black},basicstyle=\footnotesize\ttfamily\color{black}]
# Chapter 1: Introduction
Systems that automate the scientific research process, including searching literature, generating hypotheses, running experiments and writing manuscripts, are no longer just concepts. ...
\end{lstlisting}
\caption{Sequential text extraction versus intended structure, shown on the
first body page of one evaluation document. The raw extraction keeps the
visual line breaks, including mid-word hyphenation, and carries no heading
markup.}
...
structure \autocite{meuschke2023}.
