...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{architecture.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.82\textwidth,height=0.75\textheight,keepaspectratio]{classifier.png}
...
The render resolution is 150 dots per inch (DPI), set via a \texttt{fitz.Matrix(150/72,~150/72)} scale factor, which is about twice the PDF native resolution of 72 DPI.
...
Table-of-contents pages and front-matter list pages (List of Figures, List of Tables, List of Listings, and the abbreviations list) each receive a dedicated pass.
These pages are classified as TEXT and processed entirely by pymupdf4llm without invoking the LLM.
The postprocessing pass converts the pipe-table structure that pymupdf4llm produces into a clean nested Markdown list, inferring heading hierarchy from section-number prefixes, and removes dot leaders, the sequences of dots or spaces used to connect section titles to page numbers in a printed layout, which have no meaning in Markdown.
...
OCR text blocks that pymupdf4llm extracts from embedded figures and wraps in marker lines are removed. The OCR output is redundant since the figure is already included as a Markdown image link, and retaining it adds noise.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\textwidth,height=0.70\textheight,keepaspectratio]{postprocessing.png}
    \caption{Post-processing pipeline: per-page and full-document stages}
