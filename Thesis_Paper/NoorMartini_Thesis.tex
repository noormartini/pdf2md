...
files — 30 per thesis document — written manually by the thesis author.
...
Each of the four strategies — text-only, image-only, hybrid, and
adaptive — was applied to every page of every document. This produced
...
90-page corpus. Each subsection connects the findings to the relevant
research question.
...
Meuschke et al.\ \cite{meuschke2023} benchmarked ten PDF extraction
...
Blecher et al.\ \cite{blecher2023} introduced Nougat, a transformer
...
Subramani et al.\ \cite{subramani2020} surveyed document layout
...
OmniDocBench \cite{omnidocbench2025} recently showed that multi-modal
...
classifier in this system uses font-size ratios and image area
heuristics, which is effective for thesis layouts but may need
retraining for other document types.
...
\textbf{Internal validity.} The reference files were written by the
thesis author. A single annotator introduces subjectivity: different
readers may format the same page differently, and the ground truth
reflects one interpretation of what correct Markdown looks like. The
seven metrics measure structural similarity to those reference files,
not absolute correctness. Results would differ if the references were
written by a different annotator.

\textbf{Construct validity.} The metrics measure structural features
(heading counts, list counts, code block counts) rather than semantic
quality. A conversion that produces the right headings at the wrong
levels would still score well on text similarity but fail in practice.
The heading structure metric partially addresses this, but it counts
headings rather than verifying their level assignments.

\textbf{External validity.} The corpus contains three documents and
90 pages. All documents are German-language bachelor or master theses.
The strategy rankings may not hold for other document types (research
papers, reports, legal documents), other languages, or thesis documents
with a different content balance. A corpus with more formula-heavy or
image-heavy documents would likely change the relative standing of the
adaptive and text-only strategies.

\textbf{Model dependency.} All VLM strategies in this experiment use
Qwen2.5-VL-7B. A larger or differently trained model would produce
different output and could change the strategy rankings. The text-only
strategy has no such dependency: its output depends only on the PDF
and the version of pymupdf4llm, both of which are stable.
...
\begin{itemize}
    ...
    \item Refine the page classifier to distinguish simple tables from complex
          ones, so that pages with clean row-column structure bypass the vision
          LLM and use the faster text path instead
\end{itemize}
