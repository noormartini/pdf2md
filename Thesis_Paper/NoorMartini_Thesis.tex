...
correctly. Adhikari and Agarwal \cite{adhikari2024} tested ten parsing tools
across six document categories and found that even the top performers ---
PyMuPDF and pypdfium --- failed on scientific and patent documents.
...
any recognition can begin. Wei et al.\ \cite{gotocr2024} explain the
mechanical reason: traditional OCR engines can only process small image
slices, not full pages, which forces the entire pipeline into a
detect-first, recognise-second pattern where any error in region detection
propagates directly into the recognition step. Tools like
...
to HTML and other formats \cite{cone2019}. Duan \cite{cassel2025}
...
Wei et al.\ \cite{gotocr2024} proposed GOT-OCR 2.0, which they describe as a
step towards ``OCR-2.0.'' The model treats all optical signals as a single
recognition target: plain text, mathematical formulas, tables, charts, sheet
music, and geometric shapes. With 580 million parameters, a high-compression
encoder, and a long-context decoder, it handles both scene images and document
pages in whole-page and sliced input modes. Output formats include Markdown, tikz,
and SMILES notation, and the model supports region-level recognition guided by
...
Even so, key limitations remain. Adhikari and Agarwal \cite{adhikari2024} found
that even modern deep-learning tools struggle with scientific and patent documents
...
Adhikari and Agarwal \cite{adhikari2024} confirmed this in a direct comparison:
for scientific documents, tools that output structured formats such as Markdown
or \LaTeX{} outperformed all rule-based parsers, with Nougat specifically
demonstrating superior performance on these categories.
...
routing each page to the appropriate engine. Duan \cite{cassel2025}
...
Ding et al.\ \cite{ding2025} survey deep learning approaches for Visually Rich
Document Content Understanding (VRD-CU), covering documents that combine text,
layout, and visual elements. Models that integrate all three modalities through pretraining consistently
outperform text-only approaches. The most recent LLM-based architectures, trained
with instruction tuning, achieve cross-modal reasoning that earlier models could
not. But the survey also identifies persistent gaps: fine-grained word-level
representations miss higher-level semantic structure, and multi-page documents
with mixed visual and textual content remain an open challenge. For this thesis,
this means a vision LLM can handle pages where text extraction alone fails, but
no current model covers all content types reliably.
...
element type can fail in different ways. Early studies used string-similarity
metrics: edit distance (Levenshtein similarity) and BLEU score compare the
extracted text against a reference string, token by token or character by
character \cite{adhikari2024}. Adhikari and Agarwal \cite{adhikari2024} added
local alignment as a third metric alongside these two. Local alignment allows
gaps between matched segments, so it detects word-order errors --- merged
columns, reversed reading order --- that BLEU misses because BLEU counts
matching n-grams regardless of position. These metrics are easy to compute and
well understood in NLP, but they still treat the full output as a flat sequence. A table
with one column shifted will receive a low Levenshtein score even though nearly
all the content is correct.

For table evaluation, TEDS (Tree Edit Distance based Similarity) is a widely used
alternative. It compares the tree structure of a predicted table against a
reference and gives partial credit when individual cells are correct even if the
overall structure differs \cite{parsing2024}. Zhang et al.\ \cite{parsing2024} survey the range of metrics across document
parsing sub-tasks and find that no single metric covers all content types. Layout
analysis uses mean Average Precision (mAP), text extraction uses character- or
token-level edit metrics, and table evaluation uses TEDS or similar structural
measures.

The deeper problem is that text similarity scores can be high even when the
parsed output is not usable. A parser may produce text that looks close to the
reference under BLEU or edit distance but has a misaligned table header or a
dropped superscript that would cause a downstream process to fail. Zhang et al.\
\cite{parsebench2026} make this case clearly. Their ParseBench framework
evaluates parsers across five separate dimensions---table structure, chart data
extraction, content faithfulness, semantic formatting, and visual grounding---each
with its own scoring method. Across 14 parsers, no system performed well on all
five dimensions, showing that aggregate text-similarity scores hide capability
gaps that dimension-specific evaluation reveals.

For conversion to Markdown specifically, N-gram metrics such as BLEU and ROUGE
carry an additional limitation: they do not account for Markdown syntax. A heading
written with a \texttt{\#} prefix and the same text written as bold have
near-identical character content but very different semantic meaning in the
rendered document. The evaluation approach used in Chapter 4 accounts for this by
measuring structural, content, and formatting dimensions separately rather than
