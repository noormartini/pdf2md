...
layout, and visual elements. Models that integrate all three modalities through
pretraining consistently outperform text-only approaches, and the most recent
LLM-based architectures---trained with instruction tuning and prompt-driven
inference---achieve cross-modal reasoning that earlier models could not. At the
same time, the survey identifies persistent gaps: fine-grained word-level
...
character \cite{adhikari2024}. These metrics are easy to compute and well
understood in NLP, but they treat the full output as a flat sequence. A table
...
overall structure differs \cite{parsing2024}. Zhang et al.\ \cite{parsing2024}
survey the range of metrics across document parsing sub-tasks and find that no
single metric covers all content types: layout analysis uses mean Average
Precision (mAP), text extraction uses character- or token-level edit metrics, and
table evaluation uses TEDS or similar structural measures.

A more fundamental concern is that text similarity scores can be high even when
the parsed output is not usable. Zhang et al.\ \cite{parsebench2026} make this
point directly: a parser may produce text that looks close to the reference under
BLEU or edit distance but has a misaligned table header or a dropped superscript
that would cause a downstream process to fail. Their ParseBench framework
