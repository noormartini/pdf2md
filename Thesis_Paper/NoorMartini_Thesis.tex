...
The system takes a PDF file as input and produces one Markdown string per page as output.
It consists of four modules that run in sequence: a page content classifier, a PDF processing module, an LLM integration layer, and a post-processing pipeline.
...
The LLM integration layer communicates with LM Studio \cite{lmstudio} over a local HTTP server that exposes an OpenAI-compatible API \cite{khan2024}.
All LLM calls are routed through a single \texttt{call\_llm} function, which accepts the server URL, model name, message list, temperature, and token limit as parameters.
Every strategy accepts this function as an injectable parameter.
The post-processing pipeline then receives the raw LLM output and applies a fixed sequence of cleanup passes before producing the final Markdown.
...
For each TEXT page, the system calls \texttt{pymupdf4llm.to\_markdown()} with \texttt{page\_chunks=True}, which limits processing to that single page and returns the result as a dictionary containing the extracted Markdown.
Any inline figures found on the page are written to a configurable output directory, and their paths are corrected to be relative to the output file rather than the current working directory.
When multiple pages are extracted from the same document, the system can pass pre-extracted Markdown directly to avoid re-running the document-level font histogram on every call.
...
Image rendering converts each page to a base64-encoded PNG using PyMuPDF's pixmap renderer at twice the native resolution, achieved with a \texttt{fitz.Matrix(2,~2)} scale factor.
...
Two further passes remove artefacts specific to LLM output: one strips decorative horizontal rules that appear directly after a heading (PDF underlines misread as Markdown separators), and another removes prompt instruction text that some models copy verbatim into their output.
