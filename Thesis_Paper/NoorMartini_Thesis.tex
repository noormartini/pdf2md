...
Vision-Language Model (VLM) approaches. They note that robust handling of complex
...
paired PDF-markup data and can infer semantic structure, including headings,
equations, tables, and citations, without a separate layout analysis step.
This showed that a single multimodal model can replace a multi-stage OCR
...
takes this further, showing that end-to-end decoder transformer models outperform
...
pretraining consistently outperform text-only approaches. The most recent
LLM-based architectures, trained with instruction tuning, achieve cross-modal
reasoning that earlier models could not. But the survey also identifies
persistent gaps: fine-grained word-level representations miss higher-level
semantic structure, and multi-page documents with mixed visual and textual
content remain an open challenge. For this thesis, this means a vision LLM can
handle pages where text extraction alone fails, but no current model covers all
content types reliably.
...
gaps between matched segments, so it detects word-order errors, such as merged
columns or reversed reading order, that BLEU misses because BLEU counts
...
overall structure differs. Zhang et al.\ \cite{parsing2024} survey the range of
metrics across document parsing sub-tasks and find that no single metric covers
all content types. Layout analysis uses mean Average Precision (mAP), text
extraction uses character- or token-level edit metrics, and table evaluation uses
TEDS or similar structural measures.
...
\cite{parsebench2026} make this case clearly. Their ParseBench framework evaluates parsers across five separate dimensions,
each with its own scoring method: table structure, chart data extraction, content
faithfulness, semantic formatting, and visual grounding. Across 14 parsers, no system performed well on all
