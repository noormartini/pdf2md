...
rendered page image and produce structured output in a single forward pass,
with no hand-coded layout rules in between.
...
separate layout step. Adhikari and Agarwal \cite{adhikari2024} confirmed this in
a direct comparison: structured-output tools led by Nougat outperformed all
rule-based parsers on scientific documents. A single multimodal model can replace
the multi-stage pipeline, at least for academic documents.

Wang et al.\ \cite{wang2024} extended this to a broader class of documents with
MinerU, an open-source dual-engine system. MinerU detects automatically whether
each page is scanned or digitally born, routing it to either a Vision-Language
Model or a classical OCR path. It supports 109 languages and outputs structured
Markdown or JSON. Duan \cite{cassel2025} pushes the argument further:
...
Ding et al.\ \cite{ding2025} survey Visually Rich Document Content Understanding
...
\cite{adhikari2024}. Adhikari and Agarwal \cite{adhikari2024} added local
...
Zhang et al.\ \cite{parsing2024} survey metrics across document parsing
sub-tasks and conclude that no single metric covers all content types, with layout
analysis using mean Average Precision (mAP), text extraction using edit metrics,
and table evaluation using TEDS.
...
superscript that breaks any downstream process. Zhang et al.\
\cite{parsebench2026} tested 14 parsers in ParseBench across five capability
dimensions: table structure, chart data extraction, content faithfulness, semantic
formatting, and visual grounding. No system performed well on all five, confirming
...
standard for PDF parsing research. Adhikari and Agarwal \cite{adhikari2024}
applied it to benchmark seven tools across five document categories. Ingemarsson
...
rankings stay consistent across all four document categories.
...
The test corpus covers four document categories: academic papers, administrative
forms, presentation slides, and scanned archival documents. Each category
exercises a different aspect of the extraction problem. Academic papers stress
formula recognition, citation handling, and multi-column layout. Administrative
forms test table reconstruction and field alignment. Presentation slides mix
figures, bullet lists, and short text blocks on the same page. Scanned archival
documents test whether the system handles pages with no native text layer.

Documents were drawn from real sources rather than synthetic benchmarks.
Adhikari and Agarwal \cite{adhikari2024} found that tool performance varies
significantly by document type, which is why the corpus was built to include
both straightforward and structurally complex examples within each category.
The documents are primarily in German, with a subset in English, reflecting the
intended usage context. Each selected document contains at least one
structurally challenging page: a formula-heavy spread, a merged-cell table, a
figure-dense slide, or a degraded scan. The final corpus consists of
\textit{[N]}\,documents and \textit{[M]}\,pages across the four
categories.\footnote{\textit{Work in progress: the test corpus is not yet
finalised. Document and page counts will be updated before final submission.}}
...
the corpus. For each strategy, three temperature settings are tested:
0.0, 0.2, and 0.5. Temperature 0.0 produces deterministic output and
is the primary setting for comparing strategies. The two higher settings
test whether output quality varies with sampling randomness.

Two models are used. Text pages, under the text strategy and under the
adaptive strategy when it routes to text, use Qwen~3.5~9B. Visual pages,
under image-only, hybrid, and the visual path of adaptive, use
Qwen2.5-VL~7B, a vision-language model. Using the most suitable local
model for each modality reflects the real deployment scenario, where
different models can be assigned to different extraction paths.

Text-only extraction at temperature 0.0 is the baseline. It is the
simplest possible extraction method: raw text from PyMuPDF, processed by
an LLM with no image input. A strategy must outperform it on at least
two of the three metric categories to justify its additional computational
cost. Processing time and token usage are also recorded per page, providing
the raw measurements for RQ3. Table~\ref{tab:strategy_comparison}
summarises the expected strengths and weaknesses of each approach based on
the literature \cite{adhikari2024, parsing2024}.

Results are reported as mean scores per document category across all pages
in that category. Adhikari and Agarwal \cite{adhikari2024} used the same
...
\textit{arXiv preprint arXiv:1911.10683}.
