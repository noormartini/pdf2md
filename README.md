...
4. **Post-process** — 39 cleanup passes normalise headings, fix tables, remove extraction artefacts, and produce a single `.md` file
...
### Try each strategy on all three PDFs

```bash
# Sentiment analysis thesis
python3 main.py -i pdf_source/test_pdf_source.pdf -o output/sentiment_text.md    -s text
python3 main.py -i pdf_source/test_pdf_source.pdf -o output/sentiment_image.md   -s image
python3 main.py -i pdf_source/test_pdf_source.pdf -o output/sentiment_hybrid.md  -s hybrid
python3 main.py -i pdf_source/test_pdf_source.pdf -o output/sentiment_adaptive.md -s adaptive

# HITL thesis
python3 main.py -i "pdf_source/A_human_in_the_loop_system_for_research_paper_generation_using_local_large_language_models.pdf" -o output/hitl_text.md    -s text
python3 main.py -i "pdf_source/A_human_in_the_loop_system_for_research_paper_generation_using_local_large_language_models.pdf" -o output/hitl_image.md   -s image
python3 main.py -i "pdf_source/A_human_in_the_loop_system_for_research_paper_generation_using_local_large_language_models.pdf" -o output/hitl_hybrid.md  -s hybrid
python3 main.py -i "pdf_source/A_human_in_the_loop_system_for_research_paper_generation_using_local_large_language_models.pdf" -o output/hitl_adaptive.md -s adaptive

# Neural networks thesis
python3 main.py -i pdf_source/Bachelor_Thesis_Informatik_Koehler_Sven.pdf -o output/koehler_text.md    -s text
python3 main.py -i pdf_source/Bachelor_Thesis_Informatik_Koehler_Sven.pdf -o output/koehler_image.md   -s image
python3 main.py -i pdf_source/Bachelor_Thesis_Informatik_Koehler_Sven.pdf -o output/koehler_hybrid.md  -s hybrid
python3 main.py -i pdf_source/Bachelor_Thesis_Informatik_Koehler_Sven.pdf -o output/koehler_adaptive.md -s adaptive
```
...
**Requires LM Studio running with the model named in the config loaded.** The main thesis results used `qwen2.5-vl-7b-instruct-abliterated`. The shipped config lists `qwen/qwen3.6-35b-a3b` from the later model comparison (thesis chapter 7) — edit the `models` list in `experiments/sample.json` to switch.
...
311 tests cover postprocessing (182 tests), evaluation metrics, strategy logic, prompt templates, language detection, and significance testing.
...
| `-t` / `--temperature` | `0.0` | LLM sampling temperature (0.0 = deterministic) |
...
├── postprocess.py           # 39-pass Markdown cleanup pipeline
├── postprocess_test.py      # 182 unit tests for postprocessing
├── project.py               # Self-contained single-file version of the full system
├── strategies/
│   ├── adaptive.py          # Per-page classification and routing (core contribution)
│   ├── adaptive_test.py     # Tests for adaptive strategy
│   ├── text_only.py         # Text extraction strategy
│   ├── text_only_test.py    # Tests for text strategy
│   ├── image_only.py        # Vision/image strategy
│   ├── image_only_test.py   # Tests for image strategy
│   ├── hybrid.py            # Text + image combined strategy
│   ├── hybrid_test.py       # Tests for hybrid strategy
│   ├── base.py              # Shared strategy base
...
│   └── language_test.py     # Tests for language detection
├── llm/
│   ├── client.py            # LM Studio API client
│   ├── prompts.py           # Prompt templates per page type and language
│   └── prompts_test.py      # Tests for prompt templates
└── evaluation/
    ├── metrics.py           # Metric functions
    ├── metrics_test.py      # Tests for metric functions
    ├── compare.py           # Experiment runner
    ├── compare_test.py      # Tests for experiment runner
    ├── report.py            # Report generator
    ├── report_test.py       # Tests for report generator
    ├── significance.py      # Statistical significance testing
    └── significance_test.py # Tests for significance testing
```
