...
#### 1. HSMA template conversion (Thesis_Paper_2/)
Converted the original article-class thesis (`Thesis_Paper/NoorMartini_Thesis.tex`) to the HSMA HMA template format, saved as `Thesis_Paper_2/`. Key changes:
- `\documentclass{article}` → `\documentclass{HMA}` (LuaLaTeX)
- `\section{}` → `\chapter{}` hierarchy
- `\cite{}` → `\autocite{}` (BibLaTeX, numeric inline style)
- `\begin{thebibliography}` → `literatur.bib` (21 BibLaTeX entries)
- `\graphicspath{{images/}}` → `\graphicspath{{bilder/}}`
- Split into 8 chapter files: `kapitel/kapitel1.tex` through `kapitel8.tex`
- Metadata set in `docinfo.tex`: Prof. Dr. Jörn Fischer, submission 2026-07-15, faculty I, IB

Fixed compile errors:
- Removed `\usepackage{amsmath}` and `\usepackage{amssymb}` from `preambel.tex` (HMA already loads both)
- Extended `.gitignore` to cover all LaTeX build artifacts under `Thesis_Paper_2/`
- Removed staged aux/build files with `git rm --cached`

**Note:** `biber` 2.21 is installed. Compile sequence: `lualatex && biber thesis && lualatex && lualatex`.

#### 2. Architecture chapter accuracy fix (kapitel5.tex)
Compared Chapter 5 against actual code (`app.py`, `strategies/adaptive.py`, `config.py`). Fixed one inaccuracy:
- MIXED confidence score: thesis only described the 0.5 fallback; updated to describe both 0.75 (pages with image/formula signals that don't qualify for IMAGE/FORMULA/TABLE) and 0.5 (final fallback, <50 chars, no signals).
- Everything else matched the code exactly.

#### 3. Diagrams added — Chapter 5
Three Mermaid diagrams rendered to PNG and placed in `bilder/`:

| File | Figure | Section |
|---|---|---|
| `architecture.png` | 5.1 — System architecture pipeline | Section 5.1 |
| `classifier.png` | 5.2 — Page classifier decision tree | Section 5.2 |
| `postprocessing.png` | 5.3 — Post-processing pipeline (12 + 5 passes) | Section 5.5 |

Source `.mmd` files in `Thesis_Paper_2/diagrams/`. Re-render with:
```
npx @mermaid-js/mermaid-cli -i <file>.mmd -o <file>.png -w 1400 -b white
```

#### 4. Charts added — Chapter 6
Two matplotlib bar charts using real experiment data, generated by `Thesis_Paper_2/diagrams/generate_charts.py`:

| File | Figure | Content |
|---|---|---|
| `performance.png` | 6.1 — Strategy performance | Grouped bar chart, 4 strategies × 7 metrics |
| `efficiency.png` | 6.2 — Processing time | Vertical bar chart, log scale, ms values inside bars |

Re-generate: activate venv (`source venv/bin/activate`), then `cd Thesis_Paper_2/diagrams && python3 generate_charts.py`.

#### 5. Diagram added — Chapter 4
One Mermaid diagram:

| File | Figure | Section |
|---|---|---|
| `eval_pipeline.png` | 4.1 — Evaluation pipeline | Chapter 4 intro |
...
**Branch:** `noor/adaptive` — changes not yet committed since last push.
...
**Active file:** `Thesis_Paper_2/` (HSMA HMA template, LuaLaTeX, 64 pages)
**Legacy file:** `Thesis_Paper/NoorMartini_Thesis.tex` (article class, kept as reference)

**Completed chapters (Thesis_Paper_2):**
- Chapter 1 — Introduction (done)
- Chapter 2 — Foundations (done)
- Chapter 3 — Related Work (done)
- Chapter 4 — Methodology (done) + evaluation pipeline diagram
- Chapter 5 — System Design and Implementation (done) + 3 diagrams
- Chapter 6 — Results and Evaluation (done) + 2 charts
- Chapter 7 — Discussion (done)
- Chapter 8 — Conclusion — Future Work section done; three sections still empty:
  - `\section{Summary of Findings}` — empty stub
  - `\section{Contributions and Impact}` — empty stub
  - `\section{Limitations of the Study}` — empty stub

**What is left:**
- Write the three empty sections in Chapter 8 (Summary of Findings, Contributions and Impact, Limitations of the Study)
- Commit and push all Thesis_Paper_2 changes to `noor/adaptive`
- Final proofread before submission (2026-07-15)
